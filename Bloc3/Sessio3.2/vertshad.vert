...
uniform vec3 posFocus = vec3(1, 0, 1);  // en SCA
...
{	
    ...
    vec3 normalTransf = NormalMatrix * normal;
    vec3 normalSCO = normalize(normalTransf);
    vec4 focusSCO = view * vec4(posFocus,1.0);
    /*Focus de llum de camera*/
   // vec4 focusSCO = vec4(posFocus,1.0);
    vec4 L = normalize(focusSCO - vertexSCO);
    //fcolor = Lambert(normalSCO,L.xyz);
    fcolor = Phong(normalSCO, L.xyz, vertexSCO);
